...
export class ShopComponent implements OnInit {
  ...
  filterGroups: FilterGroup[] = [
    {
      label: 'Shop by Category', key: 'category', expanded: true,
      options: [
        { value: 'plants', label: 'Plants' },
        { value: 'supplies', label: 'Care Supplies' },
      ]
    },
    {
  ...
  filteredItems = computed(() => {
    ...
    let base = items.filter(p => {
      ...
      if (filters['category'].size > 0) {
        const wantsPlants = filters['category'].has('plants');
        const wantsSupplies = filters['category'].has('supplies');
        if (wantsPlants && !wantsSupplies && p.is_supply) return false;
        if (wantsSupplies && !wantsPlants && !p.is_supply) return false;
      }
  ...
  activeChips = computed(() => {
    ...
    const labelMap: Record<string, string> = {
      'plants': 'Plants', 'supplies': 'Care Supplies',
  ...
  async ngOnInit() {
    ...
    try {
      ...
      const plants: Product[] = coerceNumericFields<Product>(plantsResult.data);
      const supplies: Product[] = coerceNumericFields<Product>(suppliesResult.data);
      this.allItems.set([...plants, ...supplies]);
    } catch (err: any) {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (params['category']) {
        this.setFilter('category', params['category']);
      }
